...
export function deleteNews (_id) {
    console.log(`${url}news/delete/${_id}`);
     return axios.get(`${url}news/delete/${_id}`).then((res)=>{
        return res.data;
    }).catch(err=>console.log(err));
}

export function loadAds () {
     return axios.get(`${url}ad`).then((res)=>{
        return res.data;
    }).catch(err=>console.log(err));
}

export function getAds (_id) {
    console.log(`${url}ad/${_id}`);
     return axios.get(`${url}ad/${_id}`).then((res)=>{
        return res.data;
    }).catch(err=>console.log(err));
}

export function deleteAds (_id) {
    console.log(`${url}ad/delete/${_id}`);
     return axios.get(`${url}ad/delete/${_id}`).then((res)=>{
        return res.data;
    }).catch(err=>console.log(err));
}
